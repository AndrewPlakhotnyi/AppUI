...
function boot() {
    console.log("Booting blazor.desktop.ts...")
    setEventDispatcher((eventDescriptor, eventArgs) => DotNet.invokeMethodAsync('BlazorDesktop', 'DispatchEvent', eventDescriptor, JSON.stringify(eventArgs)));
    navigationManagerFunctions.listenForNavigationEvents((uri: string, intercepted: boolean) => {
    return DotNet.invokeMethodAsync('BlazorDesktop', 'NotifyLocationChanged', uri, intercepted);
    });
    ...
    ipc.send('BlazorDesktopJSInitialized', []);
}
boot();
